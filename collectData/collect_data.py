...
def collect_training_data(total_actions):
    ...
    num_params = 7
    # network_params used to store your training data
    # a single sample will be comprised of: sensor_readings w/ ndarray.size=5, action, collision
    network_params = []
    ...
    for action_i in range(total_actions):
        ...
        for action_timestep in range(action_repeat):
            ...
            if collision:
                ...
                # EDITS collision of PREVIOUS action if current action is very new
        ...
        # append action and collision to sensor readings ndarray
        add_col = np.append(sensor_readings, action)
        add_col = np.append(add_col, collision)
        network_params.append(add_col)

    #save actions to a csv file for use later
    # csv col_names=[sensor 1, sensor 2, sensor 3, sensor 4, sensor 5, action, collision]

    df = pd.DataFrame(network_params)
    df.to_csv('training_data.csv', header=False, index=False)

    #print(sum([x[-1] for x in network_params]))  3 find out how many collisions occured in the training set
